...
<body class="gray-bg">
                <div class="mail-box-header" style="width: 100%;height: 25%">
                    ...
                    <div class="mail-tools tooltip-demo m-t-md">
                        ...
                        <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" title="刷新列表" onclick="window.parent.mailDraft();"><i class="fa fa-refresh"></i> 刷新</button>
                        <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="top" title="移动到回收站"><i class="fa fa-trash-o"></i>
                        </button>
                        ...
                        <tbody>
                        	<c:choose>
								<c:when test="${not empty draftList}">
									<c:forEach items="${draftList}" var="var" >
										<c:if test="${var.is_read==0}">
                            				<tr class="unread" onclick="window.parent.mailEdit('${var.id}');">
										</c:if>
										<c:if test="${var.is_read==1}">
                            				<tr class="read" onclick="window.parent.mailEdit('${var.id}');">
										</c:if>
			                                <td class="check-mail">
			                                    <input type="checkbox" class="i-checks" name="ids" id="${var.id}" value="${var.id}">
			                                </td>
                                			<td class="mail-ontact" style="width:13%;">
                                				<a href="javascript:void(0);">${var.id}</a>
                                			</td>
                                			<td class="mail-ontact">
                                				<a href="javascript:void(0);">${var.send_name}</a>
                                			</td>
                                			<td class="mail-subject">
                                				<a href="javascript:void(0);">${var.title}</a>
                                			</td>
    ...
    <script type="text/javascript">
        ...
        function searchMail(){
            ...
            $('#searchForm').attr('action', "<%=basePath%>message/mailDraft.do?search="+search);
            $('#searchForm').submit();
        }
    </script>
</body>

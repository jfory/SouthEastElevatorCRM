...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						...
						<form role="form" class="form-inline" action="production/productionTwo.do"
						...
						<div class="table-responsive">
														...
														<td>
														    <c:if test="${QX.edit != 1 && QX.del != 1 }">
															   ...
															   <c:if test="${var.approval eq null || var.con_approval eq ''}">
																   ...
																   <button class="btn  btn-warning btn-sm" title="创建二排" type="button" onclick="add('${var.item_no}','${var.item_name}','${var.no}','${var.item_id}',${var.special_state},'${var.elevator_id}')">创建二排</button>
															   </c:if>
														       <!-- 待提交 -->
															   <c:if test="${var.approval eq 0 && var.special_state eq 0}">
															       <button class="btn  btn-success btn-sm" title="查看项目" type="button" onclick="edit('${var.item_id}','sel');">查看项目</button>
																   <button class="btn  btn-warning btn-sm" title="提交流程" type="button" onclick="startLeave('${var.pro_no}','${var.production_key}');">提交流程</button>
																   <button class="btn  btn-primary btn-sm" title="编辑二排" type="button" onclick="editShop('${var.pro_no}');">编辑二排</button>
															   </c:if>
															   <c:if test="${var.approval eq 0 && var.special_state eq 1}">
															       <button class="btn  btn-success btn-sm" title="查看项目" type="button" onclick="edit('${var.item_id}','sel');">查看项目</button>
																   <button class="btn  btn-warning btn-sm" title="提交流程" type="button" onclick="startLeave2('${var.pro_no}','${var.production_key}');">提交流程</button>
																   <button class="btn  btn-primary btn-sm" title="编辑二排" type="button" onclick="editShop('${var.pro_no}');">编辑二排</button>
															   </c:if>
															   <!-- 已提交  审核中-->
							...
							<div class="col-lg-12" style="padding-left: 0px; padding-right: 0px">
								<a class="btn btn-warning" title="导出到Excel" type="button" target="_blank" href="<%=basePath%>production/toExcel2.do">导出</a>
  ...
  //查看项目信息
	...
	//创建二排单
    function add(item_no,item_name,no,item_id,special_state,elevator_id) {
        $("#EditItem").kendoWindow({
            width: "800px",
            height: "450px",
            title: "创建二排单",
            actions: ["Close"],
            content: '<%=basePath%>production/goAddSTow.do?item_no='+item_no+'&item_name='+item_name+'&no='+no+'&item_id='+item_id+'&special_state='+special_state+'&elevator_id='+elevator_id,
            modal: true,
            visible: false,
            resizable: true
    ...
    function editShop(pro_no) {
        $("#EditItem").kendoWindow({
            ...
            title: "编辑排产信息",
            actions: ["Close"],
            content: '<%=basePath%>production/goEditSTow.do?pro_no=' + pro_no,
            modal: true,
            visible: false,
            resizable: true
        }).data("kendoWindow").center().open();
  ...
  //启动流程
	function startLeave(pro_no,production_key){
		swal({
			...
			if (isConfirm)
			{
				var url = "<%=basePath%>production/applytow.do?pro_no="+pro_no+'&production_key='+production_key;
	 ...
	 //特批电梯启动流程
	function startLeave2(pro_no,production_key){
		swal({
			title: "您确定要启动流程吗？",
			text: "点击确定将会启动该流程，请谨慎操作！",
			type: "warning",
			...
			if (isConfirm)
			{
				var url = "<%=basePath%>production/applytowSpecial.do?pro_no="+pro_no+'&production_key='+production_key;
				$.get(url, function (data) {
					console.log(data.msg);
					if (data.msg == "success") {
						swal({
	...
	//查看审核记录
	 ...
	 function restartAgent(task_id,pro_no){
		 swal({
			 ...
			 if (isConfirm == true) {
				 var url = "<%=basePath%>production/restartTow.do?task_id="+task_id+"&pro_no="+pro_no+"&tm="+new Date().getTime();
				 $.get(url, function (data) {
					 console.log(data.msg);
					 if (data.msg == "success") {
						 swal({
	...
	//导出到Excel
		function toExcel(){
			$.post("<%=basePath%>production/toExcel2.do");
		}
		...
		function importExcel(e){
	    	...
	    	$.ajax({
	            url:"<%=basePath%>production/importExcel2.do",
	            type:"POST",
	            data:data,
	            cache: false,
	            processData:false,
		...
		// 下载文件   e代表当前路径值 
		function downFile() {
			var url="uploadFiles/file/DataModel/Production/二排排产信息.xls";

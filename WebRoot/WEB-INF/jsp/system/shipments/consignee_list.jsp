...
<body class="gray-bg">
    <div class="wrapper wrapper-content animated fadeInRight">
        <div id="AddCarriage" class="animated fadeIn"></div>
        <div id="EditCarriage" class="animated fadeIn"></div>
	            ...
	            <div class="col-sm-12">
	                <div class="ibox float-e-margins">
	                    ...
	                    <div id="top",name="top"></div>
	                        <form role="form" class="form-inline" action="shipments/listConsignee.do" method="post" name="ConsigneeForm" id="ConsigneeForm">
	                        ...
	                        <div class="table-responsive">
	                            <table class="table table-striped table-bordered table-hover">
	                                ...
	                                <!-- 开始循环 -->	
									<c:choose>
										<c:when test="${not empty consigneeList}">
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${consigneeList}" var="var" >
													<tr>
														<%-- <td><input type="checkbox" class="i-checks" name='ids' id='${var.consignee_id}' value='${var.consignee_id}' ></td> --%>
														<td>${var.consignee_no}</td>
														<td>${var.consignee_name}</td>
														<td>${var.consignee_address}</td>
														<td>${var.consignee_contact==""?"未填":var.consignee_contact}</td>
														<td>
															${var.consignee_type==""?"未选择":""}
															${var.consignee_type=="1"?"正常":""}
															${var.consignee_type=="2"?"补发":""}
														</td>
														<td>
															${var.consignee_state==""?"未选择":""}
															${var.consignee_state=="1"?"未发货":""}
															${var.consignee_state=="2"?"装箱中":""}
															${var.consignee_state=="3"?"发货中":""}
															${var.consignee_state=="4"?"已结束":""}
														</td>
														<td>
															...
															<div>
																...
																<c:if test="${QX.edit == 1 }">
																	<button class="btn btn-sm btn-primary btn-sm" title="承运单" type="button" onclick="AddCarriage('${var.consignee_id}','edit');">添加承运单</button>
																	<button class="btn btn-sm btn-warning btn-sm" title="提交" type="button" onclick="sub('${var.consignee_id}','${var.item_id}');">发货</button>
																</c:if>
																<%-- <c:if test="${QX.del == 1 }">
    ...
    <script type="text/javascript">
		...
		//电梯列表
		function AddCarriage(consignee_id){
			$("#EditCarriage").kendoWindow({
		        width: "1000px",
		        height: "800px",
		        title: "承运单",
		        actions: ["Close"],
		        content: '<%=basePath%>shipments/goEditCarriage.do?consignee_id='+consignee_id,
		        modal : true,
				visible : false,
				resizable : true
		    }).data("kendoWindow").maximize().open();
		...
		//提交
		function sub(consignee_id,item_id){
			swal({
	                title: "您确定将出货单提交到发货管理吗？",
	                text: "提交后无法编装箱信息，请谨慎操作！",
	                type: "warning",
	                showCancelButton: true,
	                confirmButtonColor: "#DD6B55",
	            ...
	            function (isConfirm) {
	                if (isConfirm) {
	                	$.ajax({
							type: "POST",
							url: '<%=basePath%>shipments/subShipping.do',
					    	data: {

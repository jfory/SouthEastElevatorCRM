...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						...
						<form role="form" class="form-inline" action="comefund/comeFund.do"
						...
						<div class="table-responsive">
							<table class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<!-- 开始循环 -->
									<c:choose>
										<c:when test="${not empty comeFundList}">
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${comeFundList}" var="cf" varStatus="vs">
													...
													<tr>
														...
														<td>${cf.com_no}</td>
														<td>${cf.com_money}</td>
														<td>${cf.com_account}</td>
														<td>${cf.com_company}</td>
														<td>${cf.com_time}</td>
														<td>
															<c:if test="${cf.com_type eq 0 }">现金</c:if>
															<c:if test="${cf.com_type eq 1 }">额度</c:if>
															<c:if test="${cf.com_type eq 2 }">特批</c:if>
														</td>
														<td>
															<c:if test="${cf.com_states eq 0 }">未认领</c:if>
															<c:if test="${cf.com_states eq 1 }">已认领</c:if>
														</td>
														<td>${cf.com_remarks}</td>
														<td>
															...
															<c:if test="${QX.edit == 1 }">
																<button class="btn  btn-primary btn-sm" title="编辑"
																	type="button" onclick="editShop('${cf.com_no}');">编辑
																</button>
																<button class="btn  btn-primary btn-sm" title="分款"
																	type="button" onclick="claimFund('${cf.com_no}');">分款
																</button>
															</c:if> 
															<c:choose>
							...
							<div class="col-lg-12"
								...
								<a class="btn btn-warning" title="导出到Excel" type="button" target="_blank" href="<%=basePath%>comefund/toExcel.do">导出</a>
    ...
    function add() {
        $("#EditShops").kendoWindow({
            ...
            title: "新增",
            actions: ["Close"],
            content: '<%=basePath%>comefund/goAddS.do',
            modal: true,
            visible: false,
            resizable: true
    ...
    //修改
    function editShop(com_no) {
        $("#EditShops").kendoWindow({
            width: "800px",
            height: "600px",
            title: "修改单元信息",
            actions: ["Close"],
            content: '<%=basePath%>comefund/goEditS.do?com_no=' + com_no,
            modal: true,
            visible: false,
            resizable: true
        }).data("kendoWindow").center().open();
    ...
    //认领来款
    function claimFund(com_no){
        $("#EditShops").kendoWindow({
            width: "900px",
            height: "700px",
            title: "分配款项",
            actions: ["Close"],
            content: '<%=basePath%>comefund/goClaim.do?com_no=' + com_no,
            modal: true,
            visible: false,
            resizable: true
        }).data("kendoWindow").center().open();
    ...
    //删除
    function delShop(com_no) {
        swal({
                    title: "您确定要删除[" + com_no + "]吗？",
                    text: "删除后将无法恢复，请谨慎操作！",
                    type: "warning",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        var url = "<%=basePath%>comefund/delShop.do?com_no=" + com_no + "&tm=" + new Date().getTime();
            ...
            swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                type: "POST",
                                url: '<%=basePath%>comefund/deleteAllS.do',
                                data: {cell_ids: str},
                                dataType: 'json',
                                cache: false,
  ...
  //导出到Excel
	function toExcel(){
		$.post("<%=basePath%>comefund/toExcel.do");
	}
	...
	function importExcel(e){
    	...
    	$.ajax({
            url:"<%=basePath%>comefund/importExcel.do",
            type:"POST",
            data:data,
            cache: false,
            processData:false,
	...
	// 下载文件   e代表当前路径值 
	function downFile() {
		var url="uploadFiles/file/DataModel/ComeFund/来款管理.xls";

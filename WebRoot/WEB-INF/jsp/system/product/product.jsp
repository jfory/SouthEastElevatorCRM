...
<body class="gray-bg">
    <div class="wrapper wrapper-content animated fadeInRight">
        <div id="AddProduct" class="animated fadeIn"></div>
        <div id="EditProduct" class="animated fadeIn"></div>
	            ...
	            <div class="col-sm-12">
	                <div class="ibox float-e-margins">
	                    ...
	                    <div id="top",name="top"></div>
	                        <form role="form" class="form-inline" action="product/productList.do" method="post" name="productForm" id="productForm">
	                            <div class="form-group ">
	                                <input type="text" name="product_name" value="${product_name}" placeholder="这里输入名称" class="form-control">
	                            </div>
	                        ...
	                        <div class="table-responsive" >
	                            <table class="table table-striped table-bordered table-hover">
	                                <thead>
	                                    <tr>
	                                        ...
	                                        <th style="width:20%;">产品线名称</th>
	                                        <th style="width:13%;">产品线流水号</th>
	                                        <th style="width:20%;">产品线描述</th>
									...
									<c:choose>
										<c:when test="${not empty productList}">
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${productList}" var="var" varStatus="vs">
													<tr id="${var.autoid }">
														
														
														<td><input type="checkbox"  class="i-checks" name='ids' id='ids' value="${var.product_id}"></td>
														...
														<td>
																...
																-->
																<c:if test="${QX.edit == 1 }">
																	<button class="btn  btn-primary btn-sm" title="编辑" type="button"onclick="edit('${var.product_id }');">编辑</button>
																</c:if>
																<c:if test="${QX.del == 1 }">
																	<button class="btn  btn-danger btn-sm" title="删除" type="button" onclick="del('${var.product_id }','${var.product_name }');">删除</button>
																</c:if>
								...
								<div class="col-lg-12" style="padding-left:0px;padding-right:0px">
									...
									<a class="btn btn-warning" title="导出到Excel" type="button" target="_blank" href="<%=basePath%>product/toExcel.do">导出</a>
    ...
    <script type="text/javascript">
		...
		//新增
		function add(){
			$("#AddProduct").kendoWindow({
		        width: "1200px",
		        height: "700px",
		        title: "新增产品",
		        actions: ["Close"],
		        content: '<%=basePath%>product/toProductAdd.do',
		        modal : true,
				visible : false,
				resizable : true
		...
		//删除
		function del(product_id,product_name){
                swal({
                        title: "您确定要删除["+product_name+"]吗？",
                        text: "删除后将无法恢复，请谨慎操作！",
                        type: "warning",
                        ...
                        if (isConfirm) {
                        	var url = "<%=basePath%>product/productDelete.do?product_id="+product_id+"&guid="+new Date().getTime();
		...
		function edit(product_id){
			$("#EditProduct").kendoWindow({
		        width: "1200px",
		        height: "700px",
		        title: "编辑产品",
		        actions: ["Close"],
		        content: '<%=basePath%>product/toProductEdit.do?product_id='+product_id,
		        modal : true,
				visible : false,
				resizable : true
		    }).data("kendoWindow").maximize().open();
			...
			if(str==''){
				...
				swal({
	                ...
	                if (isConfirm) {
	                	$.ajax({
							type: "POST",
							url: '<%=basePath%>product/productDeleteAll.do',
					    	data: {DATA_IDS:str},
							dataType:'json',
							cache: false,
		...
		//导出到Excel
		function toExcel(){
			$.post("<%=basePath%>product/toExcel.do");
		}
		...
		function importExcel(e){
	    	...
	    	$.ajax({
	            url:"<%=basePath%>product/importExcel.do",
	            type:"POST",
	            data:data,
	            cache: false,
	            processData:false,
		...
		*/
		function downFile() {
			var url="uploadFiles/file/DataModel/Elevator/生产线管理.xls";

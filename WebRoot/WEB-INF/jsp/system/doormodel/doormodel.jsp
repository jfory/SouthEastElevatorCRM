...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						...
						<form role="form" class="form-inline" action="doormodel/doormodel.do"
							method="post" name="shopForm" id="shopForm">
							<div class="form-group ">
								<input class="form-control" id="nav-search-input" type="text"
									name="house_name" value="${pd.house_name}" placeholder="这里输入户型类型名称">
							</div>
						...
						<div class="table-responsive">
							<table class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<!-- 开始循环 -->
									<c:choose>
										<c:when test="${not empty doormodelList}">
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${doormodelList}" var="doo" varStatus="vs">
													...
													<tr>
														...
														<input
																...
																value="${doo.house_id}" id="${doo.house_id}"
																alt="${doo.house_id}" />
																<span class="lbl"></span>
																</label>
																</td>
														...
														<td>${doo.house_id}</td>
														<td>${doo.house_name}</td>
														<td>${doo.house_describe}</td>
														...
														<td><c:if test="${QX.edit != 1 && QX.del != 1 }">
																...
																<button class="btn  btn-primary btn-sm" title="编辑"
																	type="button" onclick="editShop('${doo.house_id}');">编辑
																</button>
															</c:if> <c:choose>
																...
																<c:otherwise>
																	<c:if test="${QX.del == 1 }">
																		<button class="btn  btn-danger btn-sm" title="删除"
																			type="button"
																			onclick="delShop('${doo.house_id}','${doo.house_id}');">删除
																		</button>
																	</c:if>
																</c:otherwise>
															</c:choose>
    ...
    function add() {
        $("#EditShops").kendoWindow({
            ...
            title: "户型类型信息",
            actions: ["Close"],
            content: '<%=basePath%>doormodel/goAddS.do',
            modal: true,
            visible: false,
            resizable: true
    ...
    //修改
    function editShop(house_id) {
        $("#EditShops").kendoWindow({
            width: "800px",
            height: "600px",
            title: "户型类型信息",
            actions: ["Close"],
            content: '<%=basePath%>doormodel/goEditS.do?house_id=' + house_id,
            modal: true,
            visible: false,
            resizable: true
        }).data("kendoWindow").center().open();
    ...
    function delShop(house_id,msg) {
        swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        var url = "<%=basePath%>doormodel/delShop.do?house_id=" + house_id + "&tm=" + new Date().getTime();
            ...
            swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                type: "POST",
                                url: '<%=basePath%>doormodel/deleteAllS.do',
                                data: {house_ids: str},
                                dataType: 'json',
                                cache: false,

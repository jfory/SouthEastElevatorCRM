...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						...
						<form role="form" class="form-inline" action="competitor/competitor.do"
							method="post" name="shopForm" id="shopForm">
							
						    <div class="form-group ">
								<input class="form-control" type="text" id="nav-search-input"
									type="text" name="comp_name" value="${pd.comp_name}"
									placeholder="这里输入竞争对手名称">
							</div>
							<div class="form-group ">
								<input class="form-control" id="nav-search-input" type="text"
									name="comp_address" value="${pd.comp_address}"
									placeholder="这里输入所属竞争对手地址">
							</div>
							<div class="form-group">
						...
						<div class="table-responsive">
							<table class="table table-striped table-bordered table-hover">
								<thead>
									<tr>
										...
										<th>竞争对手公司编号</th>
										<th>竞争对手公司名称</th>
										<th style="width:250px;">竞争对手公司地址</th>
										<th style="width:200px;">竞争对手公司电话</th>
										<th style="width:250px;">竞争对手公司邮箱</th>
										<th>操作</th>
									</tr>
								</thead>
								<tbody>
									<!-- 开始循环 -->
									<c:choose>
										<c:when test="${not empty competitorList}">
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${competitorList}" var="comp" varStatus="vs">
													...
													<tr>
														...
														<input
																...
																value="${comp.comp_id}" id="${comp.comp_id}"
																alt="${comp.comp_id}" />
																<span class="lbl"></span>
																</label>
																</td>
														...
														<td><c:if test="${QX.edit != 1 && QX.del != 1 }">
																...
																<button class="btn  btn-primary btn-sm" title="编辑"
																	type="button" onclick="editShop('${comp.comp_id}');">编辑
																</button>
															</c:if> <c:choose>
																...
																<c:otherwise>
																	<c:if test="${QX.del == 1 }">
																		<button class="btn  btn-danger btn-sm" title="删除"
																			type="button"
																			onclick="delShop('${comp.comp_id}','${comp.comp_name}');">删除
																		</button>
																	</c:if>
																</c:otherwise>
															</c:choose>
							...
							<div class="col-lg-12"
								...
								<a class="btn btn-warning" title="导出到Excel" type="button" target="_blank" href="<%=basePath%>competitor/toExcel.do">导出</a>
    ...
    function add() {
        $("#EditShops").kendoWindow({
            ...
            title: "竞争对手信息",
            actions: ["Close"],
            content: '<%=basePath%>competitor/goAddS.do',
            modal: true,
            visible: false,
            resizable: true
    ...
    //修改
    function editShop(comp_id) {
        $("#EditShops").kendoWindow({
            width: "800px",
            height: "600px",
            title: "竞争对手信息",
            actions: ["Close"],
            content: '<%=basePath%>competitor/goEditS.do?comp_id=' + comp_id,
            modal: true,
            visible: false,
            resizable: true
        }).data("kendoWindow").center().open();
    ...
    function delShop(comp_id,msg) {
        swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        var url = "<%=basePath%>competitor/delShop.do?comp_id=" + comp_id + "&tm=" + new Date().getTime();
            ...
            swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                type: "POST",
                                url: '<%=basePath%>competitor/deleteAllS.do',
                                data: {comp_ids: str},
                                dataType: 'json',
                                cache: false,
  ...
  //导出到Excel
	function toExcel(){
		$.post("<%=basePath%>competitor/toExcel.do");
	}
	...
	function importExcel(e){
    	...
    	$.ajax({
            url:"<%=basePath%>competitor/importExcel.do",
            type:"POST",
            data:data,
            cache: false,
            processData:false,
	...
	// 下载文件   e代表当前路径值 
	function downFile() {
		var url="uploadFiles/file/DataModel/Houses/竞争对手信息.xls";

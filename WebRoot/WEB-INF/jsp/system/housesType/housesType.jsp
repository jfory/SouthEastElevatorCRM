...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						...
						<form role="form" class="form-inline" action="housesType/housesType.do"
							method="post" name="shopForm" id="shopForm">
							<div class="form-group ">
								<input class="form-control" id="nav-search-input" type="text"
									name="type_name" value="${pd.type_name}" placeholder="这里输入建筑结构类型名称">
							</div>
						...
						<div class="table-responsive">
							<table class="table table-striped table-bordered table-hover">
								...
								<tbody>
									<!-- 开始循环 -->
									<c:choose>
										<c:when test="${not empty housestypeList}">
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${housestypeList}" var="hou" varStatus="vs">
													...
													<tr>
														...
														<td><c:if test="${QX.edit != 1 && QX.del != 1 }">
																...
																<button class="btn  btn-primary btn-sm" title="编辑"
																	type="button" onclick="editShop('${hou.id}');">编辑
																</button>
															</c:if> <c:choose>
																...
																<c:otherwise>
																	<c:if test="${QX.del == 1 }">
																		<button class="btn  btn-danger btn-sm" title="删除"
																			type="button"
																			onclick="delShop('${hou.id}','${hou.id}');">删除
																		</button>
																	</c:if>
																</c:otherwise>
															</c:choose>
							...
							<div class="col-lg-12"
								...
								<a class="btn btn-warning" title="导出到Excel" type="button" target="_blank" href="<%=basePath%>housesType/toExcel.do">导出</a>
    ...
    function add() {
        $("#EditShops").kendoWindow({
            ...
            title: "楼盘类型信息",
            actions: ["Close"],
            content: '<%=basePath%>housesType/goAddS.do',
            modal: true,
            visible: false,
            resizable: true
    ...
    function editShop(id) {
        $("#EditShops").kendoWindow({
            ...
            title: "建筑结构类型信息",
            actions: ["Close"],
            content: '<%=basePath%>housesType/goEditS.do?id=' + id,
            modal: true,
            visible: false,
            resizable: true
        }).data("kendoWindow").center().open();
    ...
    function delShop(id,msg) {
        swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        var url = "<%=basePath%>housesType/delShop.do?id=" + id + "&tm=" + new Date().getTime();
            ...
            swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            $.ajax({
                                type: "POST",
                                url: '<%=basePath%>housesType/deleteAllS.do',
                                data: {ids: str},
                                dataType: 'json',
                                cache: false,
    ...
    //导出到Excel
	function toExcel(){
		$.post("<%=basePath%>housesType/toExcel.do");
	}
	...
	function importExcel(e){
    	...
    	$.ajax({
            url:"<%=basePath%>housesType/importExcel.do",
            type:"POST",
            data:data,
            cache: false,
            processData:false,
	...
	// 下载文件   e代表当前路径值 
	function downFile() {
		var url="uploadFiles/file/DataModel/Houses/楼盘类型.xls";

...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						...
						<form role="form" class="form-inline" action="cell/cell.do"
								...
								<c:if test="${QX.add == 1 }">
								  <c:if test="${msg =='houseType'}">
									<button class="btn  btn-primary" title="新增" type="button"
										onclick="add2('${pd.houses_no}','${pd.hou_id}');">新增</button>
								  </c:if>
								  <c:if test="${msg =='cell'}">
									<button class="btn  btn-primary" title="新增" type="button"
    ...
    //(从户型请求跳转来)的新增
    function add2(houses_no,hou_id) {
        $("#EditShops").kendoWindow({
            width: "800px",
            height: "700px",
            title: "新增",
            actions: ["Close"],
            content: '<%=basePath%>cell/goAddS.do?houses_no='+houses_no+'&house_id='+hou_id,
            modal: true,
            visible: false,
            resizable: true
  ...
  //导出到Excel
	...
	// 下载文件   e代表当前路径值 
	function downFile() {
		var url="uploadFiles/file/DataModel/Houses/单元信息.xls";

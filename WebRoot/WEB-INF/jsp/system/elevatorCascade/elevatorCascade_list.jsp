...
<head>
    ...
    <script type="text/javascript">
        ...
        //获取后台参数,传给zNodes对象
        var zNodes =${elevatorCascade};
        var className = "dark";
        //编辑按钮被点击,该分部会被调用
        function beforeEditName(treeId, treeNode) {
//            editByzTree(treeNode);
            edit(treeNode.parentId,treeNode.id,treeNode.getParentNode().name);
            return false;
        }
        //删除按钮点击,该方法会被调用
            ...
            swal({
                ...
                if (result != undefined) {
                    ...
                    if (input1 == "" || input2 == "") {
                        ...
                        var url = "<%=basePath%>elevatorCascade/add.do?parentId=" + treeNode.id+ "&name=" + name + "&orderNo=" + orderNo + "&tm=" + new Date().getTime();
                        $.get(url, function (data) {
                            if (data.msg == 'success') {
                                swal({
                                    ...
                                    //后台获取最新数据,刷新zTree
                                    var url = "<%=basePath%>elevatorCascade/listElevatorCascade.do";
                                    $.get(url, function (data) {
                                        if(data.msg=="success"){
                                            zNodes = JSON.parse(data.elevatorCascade);
                ...
                if (result != undefined) {
                    ...
                    if (input1 == "" || input2 == "") {
                        ...
                        if (index1>=0||index2>=0){
                            ...
                            var url = "<%=basePath%>elevatorCascade/editElevatorCascade.do?id=" + id + "&name=" + newName + "&orderNo=" + newOrderNo + "&tm=" + new Date().getTime();
                            $.get(url, function (data) {
                                if (data.msg == 'success') {
                                    swal({
                                        title: "修改成功！",
        ...
        function delByzTree(treeNode) {
            swal({
                ...
                if (isConfirm === true) {
                    var url = "<%=basePath%>elevatorCascade/delElevatorCascade.do?id=" + treeNode.id + "&tm=" + new Date().getTime();
                    $.get(url, function (data) {
                        if (data.msg == "success") {
        ...
        function searchTreeNode(root1, root2, nodeMap, keyWord) {
            ...
            if (root2.name.toLowerCase().indexOf(keyWord.toLowerCase()) > -1 && (elevator_id==''||(elevator_id!='' && root2.nodeType==elevator_id))) {//大小写不敏感
//            if (root2.name.indexOf(keyWord) > -1) {//大小写敏感
        ...
        function refreshCurrentTab() {
            $("#elevatorCascadeForm").submit();
        }
        //新增
        function add(parentId,parentName){
            $("#AddElevatorCascade").kendoWindow({
                width: "550px",
                height: "600px",
                title: "新增功能",
                actions: ["Close"],
                content: '<%=basePath%>elevatorCascade/goAddElevatorCascade.do?parentId='+parentId+"&parentName="+parentName,
                modal : true,
                visible : false,
                resizable : true
        ...
        //修改
        function edit(parentId,id,parentName){
            $("#EditElevatorCascade").kendoWindow({
                width: "550px",
                height: "600px",
                title: "修改功能",
                actions: ["Close"],
                content: '<%=basePath%>elevatorCascade/goEditElevatorCascade.do?parentId='+parentId+"&parentName="+parentName+"&id="+id,
                modal : true,
                visible : false,
                resizable : true
            }).data("kendoWindow").center().open();
      ...
      //导出到Excel
		function toExcel(){
			$.post("<%=basePath%>elevatorCascade/toExcel.do");
		}
		...
		function importExcel(e){
	    	...
	    	$.ajax({
	            url:"<%=basePath%>elevatorCascade/importExcel.do",
	            type:"POST",
	            data:data,
	            cache: false,
	            processData:false,
...
<div class="wrapper wrapper-content animated fadeInRight">
    <div id="AddElevatorCascade" class="animated fadeIn"></div>
    <div id="EditElevatorCascade" class="animated fadeIn"></div>
    <div class="row">
        <div class="col-sm-12">
            <!-- 上方新增、按钮视图 -->
            <c:choose>
                    ...
                    <div class="ibox float-e-margins">
                        <div class="ibox-content" style="padding: 10px 10px 10px 10px;">
                            <form role="form" class="form-inline"
                                  action="elevatorCascade/listElevatorCascade.do" method="post" name="elevatorCascadeForm"
                                  id="elevatorCascadeForm">
                                ...
                                <div class="form-group">
                                    ...
                                    <a class="btn btn-warning" title="导出到Excel" type="button" target="_blank" href="<%=basePath%>elevatorCascade/toExcel.do">导出</a>
            ...
            <c:choose>
            <c:when test='${not empty elevatorCascade and (QX.cha == 1|| QX.edit == 1|| QX.del == 1||QX.add == 1)}'>display:block;</c:when>

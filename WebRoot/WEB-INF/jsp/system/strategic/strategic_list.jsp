...
<body class="gray-bg">
							...
							<div id="tab-2" class="tab-pane">
								<div class="ibox float-e-margins">
									<div class="ibox-content">
										...
										<form role="form" class="form-inline" action="strategic/strategicAudit.do" method="post" name="leaveForm2" id="leaveForm2">
															...
															<c:forEach items="${stras}" var="stra" varStatus="vs2">
																<tr>
																	...
																	<td>
																		...
																		<c:if test="${stra.stra_approval eq 2}">审核中</c:if>
																		<c:if test="${stra.stra_approval eq 3}">已取消</c:if>
																		<c:if test="${stra.stra_approval eq 4}">被驳回</c:if>
																	</td>
																	<td><a onclick="viewDiagram('${stra.stra_strategy_key}');">${stra.task_name}</a></td>
																	<td>
																		<button class="btn  btn-success btn-sm" title="查看" type="button" onclick="viewstra('${stra.stra_no}');" >查看</button>
																		...
																		<c:if test="${stra.type == '1'}">
																			<button class="btn  btn-warning btn-sm" title="办理" type="button" onclick="goHandlestra('${stra.task_id}','${stra.stra_no}');">办理</button>
																		</c:if>
																	</td>
							...
							<div id="tab-3" class="tab-pane">
								<div class="ibox float-e-margins">
									<div class="ibox-content">
										...
										<form role="form" class="form-inline" action="strategic/listDoneStra.do" method="post" name="leaveForm3" id="leaveForm3">
															...
															<c:forEach items="${strates}" var="stra" varStatus="vs3">
																<tr>
																	...
																	<td>
																		...
																		<a onclick="viewDiagram('${stra.stra_strategy_key}');">${stra.task_name}</a>
																		</c:if>
																		<c:if test="${stra.isRuning != 1}">
																			已完成
																		</c:if>
																	...
																	<td>
																		<button class="btn  btn-success btn-sm" title="查看" type="button" onclick="viewstra('${stra.stra_no}');" >查看</button>
																		<button class="btn  btn-info btn-sm" title="历史记录" type="button" onclick="viewHistory('${stra.stra_strategy_key}');">记录</button>
																	</td>
	...
	<script type="text/javascript">
		...
		function startLeave(id,process_instance_id){
		 ...
		 function claimstra(task_id){
			 swal({
				 ...
				 if (isConfirm === true) {
					 var url = "<%=basePath%>strategic/claim.do?task_id="+task_id+"&tm="+new Date().getTime();
					 $.get(url, function (data) {
						 console.log(data.msg);
						 if (data.msg == "success") {
							 swal({
	 ...
	 //办理任务
	 function goHandlestra(task_id,stra_no){
		 $("#handleLeave").kendoWindow({
			 width: "600px",
			 height: "400px",
			 title: "办理任务",
			 actions: ["Close"],
			 content: '<%=basePath%>strategic/goHandStra.do?task_id='+task_id+'&stra_no='+stra_no,
			 modal : true,
			 visible : false,
			 resizable : true

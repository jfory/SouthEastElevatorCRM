...
<body class="gray-bg">
							...
							<div id="tab-2" class="tab-pane">
								<div class="ibox float-e-margins">
									<div class="ibox-content">
										...
										<form role="form" class="form-inline" action="contractNewInstallation/ContractAudit.do" method="post" name="leaveForm2" id="leaveForm2">
										...
										<div class="table-responsive">
											<table class="table table-striped table-bordered table-hover">
												...
												<tr>
													...
													<th style="text-align:center;">合同总价格</th>
														...
														<c:if test="${QX.cha == 1 }">
															<c:forEach items="${ContractList}" var="eo" varStatus="vs2">
																<tr>
																	...
																	<td style="text-align:center;">
																		<button class="btn  btn-success btn-sm" title="查看" type="button" onclick="CNselect('${eo.AZ_UUID}')">查看</button>
																		<c:if test="${eo.type == '0'}">
																			<button class="btn  btn-primary btn-sm" title="签收" type="button" onclick="claimstra('${eo.task_id}');">签收</button>
																		</c:if>
																		<c:if test="${eo.type == '1'}">
																			<button class="btn  btn-warning btn-sm" title="办理" type="button" onclick="goHandlestra('${eo.task_id}','${eo.AZ_UUID}');">办理</button>
																		</c:if>
																	</td>
							...
							<div id="tab-3" class="tab-pane">
								<div class="ibox float-e-margins">
									<div class="ibox-content">
										...
										<form role="form" class="form-inline" action="contractNewInstallation/listDoneOffer.do" method="post" name="leaveForm3" id="leaveForm3">
														...
														<c:if test="${QX.cha == 1 }">
															<c:forEach items="${ContractList}" var="e" varStatus="vs3">
																<tr>
																	...
																	<td style="text-align:center;">
																		<button class="btn  btn-success btn-sm" title="查看" type="button" onclick="CNselect('${e.AZ_UUID}')" >查看</button>
	...
	<script type="text/javascript">
		...
		function search(id){
		 ...
		 function claimstra(task_id){
			 swal({
				 ...
				 if (isConfirm === true) {
					 var url = "<%=basePath%>contractNewInstallation/claim.do?task_id="+task_id+"&tm="+new Date().getTime();
	 ...
	 //办理任务
	 function goHandlestra(task_id,AZ_UUID){
		 $("#handleLeave").kendoWindow({
			 width: "100%",
			 height: "100%",
			 title: "办理任务",
			 actions: ["Close"],
			 content: '<%=basePath%>contractNewInstallation/goHandStra.do?task_id='+task_id+'&AZ_UUID='+AZ_UUID,
			 modal : true,
			 visible : false,
			 resizable : true

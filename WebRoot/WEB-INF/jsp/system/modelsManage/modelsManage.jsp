...
//            return canRename;
                ...
                if (result != undefined) {
                    ...
                    if (input1 == "" || input2 == "") {
                        ...
                        if (index1 >= 0 || index2 >= 0) {
                            ...
                            var url = "<%=basePath%>elevatorManage/addDepartment.do?parentId=" + treeNode.id + "&name=" + name + "&orderNo=" + orderNo + "&tm=" + new Date().getTime();
                            $.get(url, function (data) {
                                if (data.msg == 'success') {
                                    swal({
                                        ...
                                        //后台获取最新数据,刷新zTree
                                        var url = "<%=basePath%>/elevatorManage/listModels.do";
                                        $.get(url, function (data) {
                                            if (data.msg == "success") {
        ...
        //新增(顶部左上方新增按钮被点击)
        function add() {
            $("#editModel").kendoWindow({
                width: "550px",
                height: "600px",
                title: "新增梯种",
                actions: ["Close"],
                content: '<%=basePath%>elevatorManage/goAddModel.do',
                modal: true,
                visible: false,
                resizable: true
        ...
        function delByzTree(treeNode) {
            swal({
                ...
                if (isConfirm === true) {
                    var url = "<%=basePath%>elevatorManage/delModel.do?id=" + treeNode.id + "&tm=" + new Date().getTime();
                    $.get(url, function (data) {
                        if (data.msg == "success") {
                            ...
                            swal({
                                ...
                                //后台获取最新数据,刷新zTree
                                var url = "<%=basePath%>/elevatorManage/listModels";
                                $.get(url, function (data) {
                                    if (data.msg == "success") {
                                        zNodes = JSON.parse(data.departments);
                                        searchTreeNodesByKeyWord();
        ...
        function edit(id) {
            $("#editModel").kendoWindow({
                ...
                title: "修改梯种",
                actions: ["Close"],
                content: '<%=basePath%>elevatorManage/goEditModel.do?id=' + id,
                modal: true,
                visible: false,
                resizable: true
            }).data("kendoWindow").center().open();
...
<div class="wrapper wrapper-content animated fadeInRight">
    <div id="editModel" class="animated fadeIn"></div>
    <div class="row">
        <div class="col-sm-12">
            <!-- 上方新增、按钮视图 -->
            <c:choose>
                    ...
                    <div class="ibox float-e-margins">
                        <div class="ibox-content" style="padding: 10px 10px 10px 10px;">
                            <form role="form" class="form-inline"
                                  action="elevatorManage/listModels.do" method="post" name="modelForm"
                                  id="modelForm">
                                <div class="form-group ">
                                    <input autocomplete="off" id="keyWord" type="text" name="keyWord"
                                           style="max-width:150px;"
                                           placeholder="这里输入搜索关键字" class="form-control"
                                           onkeyup="searchTreeNodesByKeyWord()">
                                </div>
                                <div class="form-group">
                                    <button type="button" class="btn  btn-primary "
                                            onclick="searchTreeNodesByKeyWord();"
                                            style="margin-bottom: 0px;" title="搜索"><i style="font-size:18px;"
            ...
            <c:choose>
            <c:when test='${not empty models}'>display:block;</c:when>

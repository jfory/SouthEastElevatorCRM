...
<body class="gray-bg">
	<input type="hidden" value="${pd.consignee_id}" id="consignee_id" name="consignee_id">
												...
												<c:forEach items="${encasementList}" var="var" >
													<tr>
														...
														<td>
															...
															<div>
																<c:if test="${QX.edit == 1 }">
																	<button class="btn btn-sm btn-info btn-sm" title="查看" type="button" >查看</button>
																</c:if>
																<c:if test="${QX.edit == 1 }">
																	<button class="btn btn-sm btn-primary btn-sm" title="编辑" type="button" onclick="edit('${pd.elevator_no }','${var.encasement_id}');">编辑</button>
																</c:if>
																<c:if test="${QX.del == 1 }">
    ...
    <script type="text/javascript">
		...
		function add(elevator_no){
			...
			$("#AddEncasement").kendoWindow({
		        ...
		        title: "新增装箱",
		        actions: ["Close"],
		        content: '<%=basePath%>encasement/goAddEncasement.do?elevator_no='+elevator_no+'&consignee_id='+consignee_id,
		        modal : true,
				visible : false,
				resizable : true
		...
		//修改/查看
		function edit(elevator_no,encasement_id){
			$("#EditEncasement").kendoWindow({
		        width: "1000px",
		        height: "800px",
		        title: "编辑装箱",
		        actions: ["Close"],
		        content: '<%=basePath%>encasement/goEditEncasement.do?encasement_id='+encasement_id+'&elevator_no='+elevator_no,
		        modal : true,
				visible : false,
				resizable : true
		    }).data("kendoWindow").maximize().open();

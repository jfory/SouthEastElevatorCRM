...
<body class="gray-bg">
    <div class="wrapper wrapper-content animated fadeInRight">
        <div id="AddElevatorUnit" class="animated fadeIn"></div>
        <div id="EditElevatorUnit" class="animated fadeIn"></div>
	            ...
	            <div class="col-sm-12">
	                <div class="ibox float-e-margins">
	                    ...
	                    <div id="top",name="top"></div>
	                        <form role="form" class="form-inline" action="elevatorUnit/elevatorUnitList.do" method="post" name="elevatorUnitForm" id="elevatorUnitForm">
	                            <div class="form-group ">
	                                <input type="text" name="elevator_unit_name" value="${elevator_unit_name}" placeholder="这里输入名称" class="form-control">
	                        ...
	                        <div class="table-responsive" >
	                            <table class="table table-striped table-bordered table-hover">
	                                ...
	                                <!-- 开始循环 -->	
									<c:choose>
										<c:when test="${not empty elevatorUnitList}">
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${elevatorUnitList}" var="var" varStatus="vs">
													<tr id="${var.autoid }">
														
														
														<td><input type="checkbox"  class="i-checks" name='ids' id='ids' value="${var.elevator_unit_id}"></td>
														...
														<td>${var.elevator_unit_remark }</td>
														...
														<td>
																...
																-->
																<c:if test="${QX.edit == 1 }">
																	<button class="btn  btn-primary btn-sm" title="编辑" type="button"onclick="edit('${var.elevator_unit_id }');">编辑</button>
																</c:if>
																<c:if test="${QX.del == 1 }">
																	<button class="btn  btn-danger btn-sm" title="删除" type="button" onclick="del('${var.elevator_unit_id }','${var.elevator_unit_name }');">删除</button>
																</c:if>
								...
								<div class="col-lg-12" style="padding-left:0px;padding-right:0px">
									...
									<a class="btn btn-warning" title="导出到Excel" type="button" target="_blank" href="<%=basePath%>elevatorUnit/toExcel.do">导出</a>
											<button class="btn btn-info" title="导入到Excel" type="button" onclick="inputFile()">导入</button>
											<input style="display: none" class="form-control" type="file"  title="导入" id="provinceImportFile" onchange="importExcel(this)"/>
    ...
    <script type="text/javascript">
		...
		//新增
		function add(){
			$("#AddElevatorUnit").kendoWindow({
		        width: "1200px",
		        height: "700px",
		        title: "新增单元",
		        actions: ["Close"],
		        content: '<%=basePath%>elevatorUnit/goAddElevatorUnit.do',
		        modal : true,
				visible : false,
				resizable : true
		...
		//删除
		function del(elevator_unit_id,elevator_unit_name){
                swal({
                        title: "您确定要删除["+elevator_unit_name+"]吗？",
                        text: "删除后将无法恢复，请谨慎操作！",
                        type: "warning",
                        ...
                        if (isConfirm) {
                        	var url = "<%=basePath%>elevatorUnit/delElevatorUnit.do?elevator_unit_id="+elevator_unit_id+"&guid="+new Date().getTime();
		...
		//修改
		function edit(elevator_unit_id){
			$("#EditElevatorUnit").kendoWindow({
		        width: "1200px",
		        height: "700px",
		        title: "编辑单元",
		        actions: ["Close"],
		        content: '<%=basePath%>elevatorUnit/goEditElevatorUnit.do?elevator_unit_id='+elevator_unit_id,
		        modal : true,
				visible : false,
				resizable : true
		    }).data("kendoWindow").maximize().open();
			...
			if(str==''){
				...
				swal({
	                ...
	                if (isConfirm) {
	                	$.ajax({
							type: "POST",
							url: '<%=basePath%>elevatorUnit/elevatorUnitDeleteAll.do',
					    	data: {DATA_IDS:str},
							dataType:'json',
							cache: false,
		...
		//导出到Excel
		function toExcel(){
			$.post("<%=basePath%>elevatorUnit/toExcel.do");
		}
		...
		function importExcel(e){
	    	...
	    	$.ajax({
	            url:"<%=basePath%>elevatorUnit/importExcel.do",
	            type:"POST",
	            data:data,
	            cache: false,
	            processData:false,
		...
		*/
		function downFile() {
			var url="uploadFiles/file/DataModel/Elevator/单位管理.xls";

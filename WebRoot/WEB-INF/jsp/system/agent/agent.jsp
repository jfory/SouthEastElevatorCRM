	            ...
	            <div class="col-sm-12">
	                <div class="ibox float-e-margins">
	                    ...
	                    <div id="top",name="top"></div>
	                        <form role="form" class="form-inline" action="sysAgent/agentList.do" method="post" name="menuForm" id="menuForm">
	                            <div class="form-group ">
	                            	...
	                            	<c:if test="${QX.add == 1 }">
										<button class="btn  btn-primary" title="新增" type="button" style="margin-right: 5px;margin-bottom:0px;margin-left: 5px;" onclick="add();">新增</button>
									</c:if>
											...
											<c:if test="${QX.cha == 1 }">
												<c:forEach items="${agentList}" var="var" varStatus="vs">
													<tr id="${var.autoid }">
														...
														<c:choose>
															<c:when test="${var.contractor_instance_id ne null and (var.contractor_approval ne 0 or var.contractor_approval ne 4)}">
																<td><input type="checkbox" disabled="disabled"  class="i-checks" name='ids' id='ids' value="${var.agent_id}"></td>
															</c:when>
															<c:when test="${var.contractor_instance_id eq null and (var.agent_approval eq 0 or var.agent_approval eq 4)}">
																<td><input type="checkbox"  class="i-checks" name='ids' id='ids' value="${var.agent_id}"></td>
															</c:when>
															<c:when test="${var.contractor_instance_id eq null and (var.agent_approval ne 0 or var.agent_approval ne 4)}">
																<td><input type="checkbox" disabled="disabled"  class="i-checks" name='ids' id='ids' value="${var.agent_id}"></td>
															</c:when>
														</c:choose>
														...
														<td>
															<c:if test="${var.agent_approval eq 0 }">
																待审核
															</c:if>
															<c:if test="${var.agent_approval eq 1 }">
																审核中
															</c:if>
															<c:if test="${var.agent_approval eq 2 }">
																已完成
															</c:if>
															<c:if test="${var.agent_approval eq 3}">
																已取消
															</c:if>
															<c:if test="${var.agent_approval eq 4 }">
																被驳回
															</c:if>
														</td>
														...
														<td>
															...
															<div>
																...
																<c:if test="${var.agent_approval eq 0 }">
																	...
																	<c:if test="${var.agent_instance_id ne null }">	
																		<button class="btn  btn-warning btn-sm" title="提交" type="button" onclick="startAgent('${var.agent_id }','${var.agent_instance_id}','${var.contractor_instance_id }');">提交</button>
																	</c:if>	
																	</c:if>
																	<c:if test="${QX.del == 1 }">
																		<c:choose>
																			<c:when test="${var.contractor_instance_id ne null and (var.contractor_approval eq 0 or var.contractor_approval eq 4)}">
																				<button class="btn  btn-danger btn-sm" title="删除" type="button" onclick="del('${var.agent_id }','${var.agent_name }');">删除</button>
																			</c:when>
																			<c:when test="${var.contractor_instance_id eq null}">
																				<button class="btn  btn-danger btn-sm" title="删除" type="button" onclick="del('${var.agent_id }','${var.agent_name }');">删除</button>
																			</c:when>
																		</c:choose>
																	</c:if>
																...
																<c:if test="${var.agent_approval eq 1 }">
																	<c:if test="${QX.cha == 1 }">
																		<button class="btn  btn-success btn-sm" title="查看" type="button" onclick="viewAgent('${var.agent_id }');" >查看</button>
																		<button class="btn  btn-info btn-sm" title="历史记录" type="button" onclick="viewHistory('${var.agent_instance_id}');">代理商记录</button>
																	</c:if>
																</c:if>
																<!-- 已完成 -->
																<c:if test="${var.agent_approval eq 2}">
																...
																<c:if test="${var.agent_approval eq 4 }">
																	<c:if test="${QX.edit == 1 }">
																		<button class="btn  btn-primary btn-sm" title="编辑" type="button"onclick="edit('${var.agent_id }');">编辑</button>
																		<button class="btn  btn-warning btn-sm" title="重新申请" type="button" onclick="restartAgent('${var.task_id }','${var.agent_id }');">重新申请代理商</button>
																	</c:if>
																	<c:if test="${QX.cha == 1 }">
																	<button class="btn  btn-info btn-sm" title="历史记录" type="button" onclick="viewHistory('${var.agent_instance_id}');">代理商记录</button>
																	</c:if>
    ...
    <script type="text/javascript">
		...
		function add(){
			$("#AddAgent").kendoWindow({
		        ...
		        title: "新增代理商",
		        actions: ["Close"],
		        content: '<%=basePath%>sysAgent/toAdd.do',
		        modal : true,
				visible : false,
				resizable : true
		...
		function edit(agent_id){
			$("#EditAgent").kendoWindow({
		        ...
		        title: "编辑代理商",
		        actions: ["Close"],
		        content: '<%=basePath%>sysAgent/toEdit.do?agent_id='+agent_id,
		        modal : true,
				visible : false,
				resizable : true
		    }).data("kendoWindow").maximize().open();
		...
		//重新申请分包商
		 function restartContractor(task_id2,agent_id){
			 ...
			 swal({
				 ...
				 if (isConfirm === true) {
					 var url = "<%=basePath%>sysAgent/restartAgent.do?task_id2="+task_id2+"&agent_id="+agent_id+"&tm="+new Date().getTime();
					 $.get(url, function (data) {
						 console.log(data.msg);
						 if (data.msg == "success") {
							 swal({
		...
		//导出到Excel
			...
			// 下载文件   e代表当前路径值 
			function downFile() {
				var url="uploadFiles/file/DataModel/Agent/代理商.xls";

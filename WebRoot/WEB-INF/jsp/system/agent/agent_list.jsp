...
<body class="gray-bg">
							...
							<div id="tab-2" class="tab-pane">
								<div class="ibox float-e-margins">
									<div class="ibox-content">
										...
										<form role="form" class="form-inline" action="sysAgent/listPendingAgent.do" method="post" name="leaveForm2" id="leaveForm2">
														...
														<c:if test="${QX.cha == 1 }">
															<c:forEach items="${agents}" var="agent" varStatus="vs2">
																<tr>
																	...
																	<td>
																		<c:if test="${agent.agent_type eq 1 and agent.is_constructor eq 1}">
																			代理商有安装资质
																		</c:if>
																		<c:if test="${agent.agent_type eq 1 and agent.is_constructor eq 2}">
																			代理商
																		</c:if>
																		
																	</td>
																	<td>
																		<c:if test="${agent.agent_approval eq 0}">待申请</c:if>
																		<c:if test="${agent.agent_approval eq 1}">待审批</c:if>
																		<c:if test="${agent.agent_approval eq 2}">已完成</c:if>
																		<c:if test="${agent.agent_approval eq 3}">已取消</c:if>
																		<c:if test="${agent.agent_approval eq 4}">被驳回</c:if>
																	</td>
																	<td><a onclick="viewDiagram('${agent.agent_instance_id}');">${agent.task_name }</a></td>
																	<td>
																		...
																		<c:if test="${agent.type == '1'}">
																			<button class="btn  btn-warning btn-sm" title="办理" type="button" onclick="goHandleAgent('${agent.task_id}','${agent.agent_id }','${agent.agent_instance_id}');">办理</button>
																		</c:if>
																	</td>
							...
							<div id="tab-3" class="tab-pane">
								<div class="ibox float-e-margins">
									<div class="ibox-content">
										...
										<form role="form" class="form-inline" action="sysAgent/listDoneAgent.do" method="post" name="leaveForm3" id="leaveForm3">
														...
														<c:if test="${QX.cha == 1 }">
															<c:forEach items="${dleaves}" var="dleave" varStatus="vs3">
																<tr>
																	...
																	<td>
																		<c:if test="${dleave.agent_type eq 1 and dleave.is_constructor eq 1}">
																			代理商有安装资质
																		</c:if>
																		<c:if test="${dleave.agent_type eq 1 and dleave.is_constructor eq 2}">
																			代理商
																		</c:if>
																	</td>
																	<td>
																		<c:if test="${dleave.agent_approval eq 0}">待申请</c:if>
																		<c:if test="${dleave.agent_approval eq 1}">待审批</c:if>
																		<c:if test="${dleave.agent_approval eq 2}">已完成</c:if>
																		<c:if test="${dleave.agent_approval eq 3}">已取消</c:if>
																		<c:if test="${dleave.agent_approval eq 4}">被驳回</c:if>
																	</td>
																	<td>
																		<c:if test="${dleave.isRuning == 1}">
																		<a onclick="viewDiagram('${dleave.agent_instance_id}');">${dleave.task_name }</a>
																		</c:if>
																		<c:if test="${dleave.isRuning != 1}">
																			已完成
																		</c:if>
																	...
																	<td>
																		...
																		<button class="btn  btn-info btn-sm" title="历史记录" type="button" onclick="viewHistory('${dleave.agent_instance_id}');">记录</button>
																	</td>
	...
	<script type="text/javascript">
	 ...
	 //办理任务
	 function goHandleAgent(task_id,agent_id,agent_instance_id){
		 $("#handleLeave").kendoWindow({
		     width: "1150px",
		     height: "700px",
			 title: "办理任务",
			 actions: ["Close"],
			 content: '<%=basePath%>sysAgent/goHandleAgent?task_id='+task_id+'&agent_id='+agent_id+'&agent_instance_id='+agent_instance_id,
			 modal : true,
			 visible : false,
			 resizable : true
		 }).data("kendoWindow").center().open();

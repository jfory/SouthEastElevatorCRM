...
<body class="gray-bg">
    <div class="wrapper wrapper-content animated fadeInRight">
    	<div id="Viewstra" class="animated fadeIn"></div>
							...
							<div id="tab-2" class="tab-pane">
								<div class="ibox float-e-margins">
									<div class="ibox-content">
										...
										<form role="form" class="form-inline" action="production/TowSpecialApproval.do" method="post" name="leaveForm2" id="leaveForm2">
										...
										<div class="table-responsive">
											<table class="table table-striped table-bordered table-hover">
												...
												<!-- 开始循环 -->
												<c:choose>
													<c:when test="${not empty stras}">
														<c:if test="${QX.cha == 1 }">
															<c:forEach items="${stras}" var="stra" varStatus="vs2">
																<tr>
																	<td class='center' style="width: 30px;">${vs2.index+1}</td>
																	<td>${stra.pro_no}</td>
																	<td>${stra.item_name}</td>
																	<td>${stra.elevator_no}</td>
																	<td>${stra.elevator_name}</td>
																	<td>${stra.product_name}</td>
																	<td>${stra.total}</td>
																	<td>
																		<c:if test="${stra.approval eq 0}">待启动</c:if>
																		<c:if test="${stra.approval eq 1}">审核中</c:if>
																		<c:if test="${stra.approval eq 2}">审核中</c:if>
																		<c:if test="${stra.approval eq 3}">已取消</c:if>
																		<c:if test="${stra.approval eq 4}">被驳回</c:if>
																	</td>
																	<td><a onclick="viewDiagram('${stra.production_key}');">${stra.task_name}</a></td>
																	<td>
																		<button class="btn  btn-success btn-sm" title="查看" type="button" onclick="viewstra('${stra.pro_no}');" >查看</button>
																		<c:if test="${stra.type == '0'}">
																			<button class="btn  btn-primary btn-sm" title="签收" type="button" onclick="claimstra('${stra.task_id}');">签收</button>
																		</c:if>
																		<c:if test="${stra.type == '1'}">
																			<button class="btn  btn-warning btn-sm" title="办理" type="button" onclick="goHandlestra('${stra.task_id}','${stra.pro_no}');">办理</button>
																		</c:if>
																	</td>
							...
							<div id="tab-3" class="tab-pane">
								<div class="ibox float-e-margins">
									<div class="ibox-content">
										...
										<form role="form" class="form-inline" action="production/TowlistSpecial.do" method="post" name="leaveForm3" id="leaveForm3">
										...
										<div class="table-responsive">
											<table class="table table-striped table-bordered table-hover">
												...
												<!-- 开始循环 -->
												<c:choose>
													<c:when test="${not empty strates}">
														<c:if test="${QX.cha == 1 }">
															<c:forEach items="${strates}" var="stra" varStatus="vs3">
																<tr>
																	...
																	<td>
																		<c:if test="${stra.isRuning == 1}">
																		<a onclick="viewDiagram('${stra.production_key}');">${stra.task_name}</a>
																		</c:if>
																		<c:if test="${stra.isRuning != 1}">
																			已完成
																		</c:if>
																	...
																	<td>
																		<button class="btn  btn-success btn-sm" title="查看" type="button" onclick="viewstra('${stra.pro_no}');" >查看</button>
																		<button class="btn  btn-info btn-sm" title="历史记录" type="button" onclick="viewHistory('${stra.production_key}');">审核记录</button>
																	</td>
	...
	<script type="text/javascript">
		...
		function startLeave(id,process_instance_id){
		 ...
		 function claimstra(task_id){
			 swal({
				 ...
				 if (isConfirm === true) {
					 var url = "<%=basePath%>production/claimTow.do?task_id="+task_id+"&tm="+new Date().getTime();
					 $.get(url, function (data) {
						 console.log(data.msg);
						 if (data.msg == "success") {
							 swal({
	 ...
	 //办理任务
	 function goHandlestra(task_id,pro_no){
		 $("#handleLeave").kendoWindow({
			 width: "600px",
			 height: "400px",
			 title: "办理任务",
			 actions: ["Close"],
			 content: '<%=basePath%>production/goHandleTowSpecial.do?task_id='+task_id+'&pro_no='+pro_no,
			 modal : true,
			 visible : false,
			 resizable : true
	...
	//查看 详细信息 
		function viewstra(pro_no){
			$("#Viewstra").kendoWindow({
		        width: "800px",
		        height: "400px",
		        title: "特批排产信息",
		        actions: ["Close"],
		        content: '<%=basePath%>production/toViewTow.do?pro_no='+pro_no,
		        modal : true,
				visible : false,
				resizable : true
		    }).data("kendoWindow").center().open();

...
<body class="gray-bg">
			...
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						...
						<form role="form" class="form-inline" action="production/elevatorDetails.do"
							...
							<button class="btn  btn-success" title="刷新" type="button"
								style="float: right" onclick="refreshCurrentTab('${pd.item_id}');">刷新
							</button>
						</form>
						<div class="row">
							</br>
														...
														<td>
														    <c:if test="${QX.edit != 1 && QX.del != 1 }">
															   ...
															   <c:if test="${var.approval eq null || var.con_approval eq ''}">
																   ...
																   <button class="btn  btn-warning btn-sm" title="创建特批" type="button" onclick="add('${var.con_item_no}','${var.item_name}','${var.no}','${var.item_id}','${var.elevator_id}')">创建特批</button>
															   </c:if>
														       <!-- 待提交 -->
															   <c:if test="${var.approval eq 0}">
																   ...
																   <button class="btn  btn-primary btn-sm" title="编辑特批" type="button" onclick="editShop('${var.pro_no}');">编辑特批</button>
															   </c:if>
															   <!-- 已提交  审核中-->
    ...
    //刷新iframe
    function refreshCurrentTab(item_id) {
        $("#shopForm").submit();
    }
    //检索
    function search() {
  ...
  //查看项目信息
    ...
    function add(con_item_no,item_name,no,item_id,elevator_id) {
        $("#EditItem").kendoWindow({
            ...
            title: "创建特批",
            actions: ["Close"],
            content: '<%=basePath%>production/AddSpecial.do?con_item_no='+con_item_no+'&item_name='+item_name+'&no='+no+'&item_id='+item_id+'&elevator_id='+elevator_id,
            modal: true,
            visible: false,
            resizable: true
  ...
  //启动流程
	function startLeave(pro_no,production_key){
		swal({
			...
			if (isConfirm)
			{
				var url = "<%=basePath%>production/applySpecial.do?pro_no="+pro_no+'&production_key='+production_key;
	...
	//查看审核记录
	 ...
	 function restartAgent(task_id,pro_no){
		 swal({
			 ...
			 if (isConfirm == true) {
				 var url = "<%=basePath%>production/restartSpecial.do?task_id="+task_id+"&pro_no="+pro_no+"&tm="+new Date().getTime();
				 $.get(url, function (data) {
					 console.log(data.msg);
					 if (data.msg == "success") {
						 swal({

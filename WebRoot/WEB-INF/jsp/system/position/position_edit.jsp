	...
	<div class="wrapper wrapper-content">
		<div class="row">
			<div class="col-sm-12">
				...
				<%--用户ID--%>
				<form action="position/${msg }.do" name="positionForm" id="positionForm" method="post">
				...
				<div class="ibox float-e-margins">
							...
							<div class="col-lg-12">
								...
								<c:choose>
									<c:when test="${not empty pd.parentId}">
										<input type="text" value="${pd.parentName}" readonly id="departmentSelect" class="form-control" placeholder="请点击选择所属部门" onclick="showMenu();">
									</c:when>
									<c:otherwise>
										<input type="text" value="" readonly id="departmentSelect" class="form-control" placeholder="请点击选择所属部门" onclick="showMenu();">
									</c:otherwise>
								</c:choose>
							    </div>
								<div class="form-group form-inline" style="margin-top: 15px;">
								...
								<div class="form-group form-inline" style="margin-top: 15px;">
									<label>是否为主管职位:</label>
									<select class="form-control" name="is_manager" id="is_manager" data-placeholder="请选择是否为主管职位"  title="是否为主管职位">
											<option value="" >是否为主管职位</option>
											<option value=1 <c:if test="${pd.is_manager == 1 }">selected</c:if>>是</option>
											<option value=0 <c:if test="${pd.is_manager == 0 }">selected</c:if>>否</option>
									</select>
					...
					<tr>
						...
						<c:if test="${msg eq 'addPosition'}">
							<td><a class="btn btn-danger" style="width: 150px; height: 34px;float:right;" onclick="javascript:CloseSUWin('AddPosition');">关闭</a></td>
						</c:if>
						<c:if test="${msg eq 'editPosition'}">
						<td><a class="btn btn-danger" style="width: 150px; height: 34px;float:right;" onclick="javascript:CloseSUWin('EditPosition');">关闭</a></td>
						</c:if>
					</tr>
				</div>
			</form>

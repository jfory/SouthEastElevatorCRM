...
//            editByzTree(treeNode);
        ...
        function showRemoveBtn(treeId, treeNode) {
            return (treeNode.nodeType=="position") && canRemove;
        }
        //修改权限
        var canRename = (${QX.edit} == 1)? true : false;
        //显示修改按钮
        function showRenameBtn(treeId, treeNode) {
            return (treeNode.nodeType=="position")&&canRename;
        }
        //获取系统时间
        function getTime() {
            var now = new Date(),
            ...
            swal({
                ...
                if (result != undefined) {
                    ...
                    if (input1 == "" || input2 == "") {
                        ...
                        var url = "<%=basePath%>position/add.do?parentId=" + treeNode.id+ "&name=" + name + "&orderNo=" + orderNo + "&tm=" + new Date().getTime();
                        $.get(url, function (data) {
                            if (data.msg == 'success') {
                                swal({
                                    ...
                                    //后台获取最新数据,刷新zTree
                                    var url = "<%=basePath%>position/getAllPositions.do";
                                    $.get(url, function (data) {
                                        if(data.msg=="success"){
                                            zNodes = JSON.parse(data.positions);
                ...
                if (result != undefined) {
                    ...
                    if (input1 == "" || input2 == "") {
                        ...
                        if (index1>=0||index2>=0){
                            ...
                            var url = "<%=basePath%>position/editPosition.do?id=" + id + "&name=" + newName + "&orderNo=" + newOrderNo + "&tm=" + new Date().getTime();
                            $.get(url, function (data) {
                                if (data.msg == 'success') {
                                    swal({
                                        title: "修改成功！",
        ...
        function delByzTree(treeNode) {
            swal({
                ...
                if (isConfirm === true) {
                    var url = "<%=basePath%>position/delPosition.do?id=" + treeNode.id + "&tm=" + new Date().getTime();
                    $.get(url, function (data) {
                        if (data.msg == "success") {
...
//            if (root2.name.indexOf(keyWord) > -1) {//大小写敏感
        ...
        function add(parentId,parentName){
            $("#AddPosition").kendoWindow({
                width: "550px",
                height: "600px",
                title: "新增职位",
                actions: ["Close"],
                content: '<%=basePath%>position/goAddPosition.do?parentId='+parentId+"&parentName="+parentName,
                modal : true,
                visible : false,
                resizable : true
        ...
        function edit(parentId,id,parentName){
            $("#EditPosition").kendoWindow({
                width: "550px",
                height: "600px",
                title: "修改职位",
                actions: ["Close"],
                content: '<%=basePath%>position/goEditPosition.do?parentId='+parentId+"&parentName="+parentName+"&id="+id,
                modal : true,
                visible : false,
                resizable : true
            }).data("kendoWindow").center().open();
        ...
        function importExcel(e){
            var url = "<%=basePath%>position/importExcelPosition.do";
            var filePath = $(e).val();
...
<div class="wrapper wrapper-content animated fadeInRight">
    <div id="AddPosition" class="animated fadeIn"></div>
    <div id="EditPosition" class="animated fadeIn"></div>
    <div class="row">
        <div class="col-sm-12">
            <!-- 上方新增、按钮视图 -->
            <c:choose>
                    ...
                    <div class="ibox float-e-margins">
                        <div class="ibox-content" style="padding: 10px 10px 10px 10px;">
                            <form role="form" class="form-inline"
                                  action="position/listPositions.do" method="post" name="positionForm"
                                  id="positionForm">
                                ...
                                <div class="form-group">
                                    ...
                                    <a id="alink_toExcel" target="_blank" href="<%=basePath%>position/toExcelPosition.do" style="display: none"><span></span></a>
                                </div>
                                <div class="form-group">
                                    <button type="button" class="btn btn-primary btn-outline" onclick="importPosition();"
            ...
            <c:choose>
            <c:when test='${not empty positions and (QX.cha == 1|| QX.edit == 1|| QX.del == 1||QX.add == 1)}'>display:block;</c:when>

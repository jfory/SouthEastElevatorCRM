	            ...
	            <div class="col-sm-12">
	                <div class="ibox float-e-margins">
	                    ...
	                    <div id="top",name="top"></div>
	                        <form role="form" class="form-inline" action="models/modelsList.do" method="post" name="modelsForm" id="modelsForm">
	                            <div class="form-group ">
	                                <input type="text" name="models_name" value="${models_name}" placeholder="这里输入名称" class="form-control">
	                                <select  class="form-control" name="elevator_id" id="elevator_id" title="型号类型">
	                               			<option value="">请选择型号类型</option>
	                               			<c:forEach items="${elevatorList }" var="elevator">
	                               				<option value="${elevator.elevator_id }" <c:if test="${elevator.elevator_id eq page.pd.elevator_id }">selected</c:if> >${elevator.elevator_name }</option>
	                               			</c:forEach>
									</select>
	                            </div>
	                        ...
	                        <div class="table-responsive" >
								...
								<div class="col-lg-12" style="padding-left:0px;padding-right:0px">
									...
									<a class="btn btn-warning" title="导出到Excel" type="button" target="_blank" href="<%=basePath%>models/toExcel.do?elevator_id=1">导出</a>
											<button class="btn btn-info" title="导入到Excel" type="button" onclick="inputFile()">导入</button>
											<input style="display: none" class="form-control" type="file"  title="导入" id="importFile" onchange="importExcel(this,1)"/>
										${page.pageStr}
								</div>
	                        </div>
	                    </div>

					...
					<div class="panel-body">
						<div class="tab-content">
										...
										<div class="panel panel-primary">
						                    ...
						                    <div class="form-group form-inline">
			                                 	...
			                                 	<label style="width:10%;">
			                                     	...
			                                     	placeholder="请输入预计信用证收款日期" title="预计信用证收款日期" class="form-control">
			                               			
						                    	<label style="width:10%;">
			                              			<span><font color="red"></font></span> 买断:</label>	
			                                 		      <input type="checkbox" name="HT_SFMD_TEXT" disabled="disabled" id="HT_SFMD_TEXT"  value="是" ${pd.HT_SFMD=='1'?'checked':''} />
						                    ...
						                    <div class="form-group form-inline">
						                    	<label style="width:10%;">申请人:</label>
		                                     	<label style="width:22%;">${pd.user_name}</label>
								...
								<div class="panel-body">
									<div id="fileLDiv">
										...
										<label style="width:10%;">
											...
											<button style="margin-left:3px;margin-top:3px;" class="btn  btn-primary btn-sm isaddbtn"
									...
									<div class="form-group form-inline" id="JFHT">
										...
										<label style="width:5%;">
											技术:</label>
										<textarea name="HT_JF_JS" readonly="readonly" id="HT_JF_JS" cols="30" rows="2" style="width:18%;" class="form-control">${pd.HT_JF_JS}</textarea>
										<label style="width:5%;">
											安装:</label>
										<textarea name="HT_JF_AZ" readonly="readonly" id="HT_JF_AZ" cols="30" rows="2" style="width:18%;" class="form-control">${pd.HT_JF_AZ}</textarea>
										<label style="width:5%;">
											维保:</label>
										<textarea name="HT_JF_WB" readonly="readonly" id="HT_JF_WB" cols="30" rows="2" style="width:18%;" class="form-control">${pd.HT_JF_WB}</textarea>
										<label style="width:5%;">
											财务相关:</label>
										<textarea name="HT_JF_CWXG" readonly="readonly" id="HT_JF_CWXG" cols="30" rows="2" style="width:18%;" class="form-control">${pd.HT_JF_CWXG}</textarea>
							...
							<!-- 电梯信息 End -->
								...
								<div class="row">
								<form action="contractNew/handleAgent.do" name="handleLeaveForm" id="handelLeaveForm" method="post">
	<input type="hidden" id="task_id" name="task_id" value="${pd.task_id}" readonly="readonly" >
	<input type="hidden" id="HT_UUID" name="HT_UUID" value="${pd.HT_UUID}" readonly="readonly" >
	<input type="hidden" id="action" name="action" value="${pd.action}" readonly="readonly" >
	<input type="hidden" name="HT_FJSC_JSON_JS" id="HT_FJSC_JSON_JS" value="">
	<input type="hidden" name="HT_FJSC_JS" id="HT_FJSC_JS" value="">
	<input type="hidden" name="HT_WJLX_JS" id="HT_WJLX_JS" value="">
			<div class="col-sm-12">
				<div class="ibox float-e-margins">
					<div class="ibox-content">
						<div class="row">
                            ...
                            <div class="col-lg-12">
                                <input type="hidden" name="dfkey" id="dfkey" value="${pd.dfkey}">
                                <div class="form-group form-inline" style="${pd.dfkey=='businesstask'?'':'display:none;'}">
                                    <label style="width:12%;">
                                        <span style="color: #FF0000" color="red">*</span>是否维保及法务评审:</label>
                                    <select style="width:20%;" class="form-control" name="nextlevel" id="nextlevel" title="是否需要下级审批">
                                        <option value="">请选择</option>
                                        <option value="0">不需要</option>
                                        <option value="1">需要</option>
                                    </select>
                                </div>
                                <div class="form-group form-inline" style="${pd.dfkey=='engineetask' && pd.HT_SFMD !='1'?'':'display:none;'}">
<!--                                     <label style="width:10%;"> -->
<!--                                         <span style="color: #FF0000" color="red">*</span>毛利润:</label> -->
<!--                                     <select style="width:20%;" class="form-control" name="profit" id="profit" title="毛利润"> -->
<!--                                         <option value="">请选择</option> -->
<!--                                         <option value="0">低于32%</option> -->
<!--                                         <option value="1">高于32%</option> -->
<!--                                     </select> -->
<!-- 										<input type="text" name="profit" id="profit" placeholder="这里输入毛利率" /> -->
										<label> 毛利润:</label>&nbsp;&nbsp;
										<textarea class="form-group" style="width:12%;" name="profit" id="profit"  placeholder="这里输入毛利润" maxlength="10" title="毛利率" onblur="check(this)"  onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')" ></textarea>
                                </div>
                            </div>
						</div>
					</div>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
        ...
        /* if("${pd.dfkey}" == 'techtask' && $("#fileLJSDiv").find("div").length == 0){
        	var s = '<div class="form-group form-inline">'
					+'<label style="width:10%;">'
					+'<span></span>附件上传:</label>'
		...
		if('${msg}'=='view'){
            ...
            if("${pd.dfkey}" != 'techtask'){
                $("#fileLDiv").find("button").attr("disabled","disabled");
                $("#fileLDiv").find("input[type=file]").attr("disabled","disabled");
                $("#fileLJSDiv").find("button").attr("disabled","disabled");
                $("#fileLJSDiv").find("input[type=file]").attr("disabled","disabled");
            } else {
                $("#fileLDiv").find("button").attr("disabled","disabled");
                $('.isaddbtn').removeAttr("disabled");
                $("#fileLDiv").find("input[type=file]").attr("disabled","disabled");
            	//$("#fileLDiv").find("select").removeAttr("disabled");
            	//$("#fileLJSDiv").find("select").removeAttr("disabled");
            }
//             $("#profit").removeAttr("disabled");
	...
	function setHtFjscJSON(htfjscjson){
        	...
        	if(obj[j].HT_JSFJ && obj[j].HT_JSFJ == '1'){
        		...
        		$("#fileLDiv").find("div:last").each(function(){
            		...
            		if(obj[j].HT_WJLX == "4"){
            			...
            			if("${pd.dfkey}"=="techtask"){
            				$(this).append('<label style="margin-left: 10px;"><input class="i-checks" type="checkbox" name="jsisdown" /><span style="padding-left: 3px;">已查看甲方合同</span></label>');
            	        }
            		}
                });
        ...
        if("${pd.dfkey}"=="businesstask" && isHasJFHT){
        	$('#nextlevel').val("1");
        }
	...
	function save() 
	{
        ...
        $("#fileLJSDiv").find("div").each(function(){
            ...
            var seltype=$(this).find("select").eq(0).val();
            json += "{\'HT_JSFJ\':\'1\',\'HT_FJSC\':\'"+h+"\',\'HT_WJLX\':\'"+seltype+"\',\'FILENAME\':\'"+fname+"\'},";
        });
  ...
  //保存
	function handle(action) {
	    if(action=="approve"){
			...
			}else if(dfkey=="engineetask" && "${pd.HT_SFMD}" !='1'){
                if($("#profit").val()==""){
                    $("#profit").focus();
                    $("#profit").tips({
                        side: 3,
  	...
  	function setJSFJ(action) {
		var vaFj = true;
  		var h="";
        var hjs="";
        var json = "[";
        //附件拼接为json格式保存
        $("#fileLDiv").children("div").each(function(){
            h = $(this).find("input").eq(0).val();
            var fname= $(this).find("[name='FILENAME']").eq(0).val();

            var seltype=$(this).find("select").eq(0).val();
            
            if(seltype == "4" && action=="approve" && "${pd.dfkey}" == 'techtask'){
            	if(!$(this).find("input[name=jsisdown]").is(':checked')){
            		$(this).find("input[name=jsisdown]").focus();
            		$(this).find("input[name=jsisdown]").tips({
        				side : 3,
        				msg : "请下载甲方合同进行查看，确认后勾选",
        				bg : '#AE81FF',
        				time : 3,
                        x:document.body["scrollTop"] | document.documentElement["scrollTop"]
        			});
        ...
        {
            $("#HT_FJSC_JSON_JS").val(json);
        }
        $('#HT_FJSC_JS').val($('[name=HT_FJSC]').eq(0).val());
        $('#HT_WJLX_JS').val($('[name=HT_WJLX]').eq(0).val());

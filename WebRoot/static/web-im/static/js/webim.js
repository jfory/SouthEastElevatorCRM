...
var handleOpen = function(conn) {
			...
			conn.listRooms({
				success : function(rooms) {
					...
					if (rooms && rooms.length > 0) {
						...
						buildListRoomJson(rooms);//群组列表json
	...
	conn.getChatRooms({
		...
		success: function ( list ) {
			...
			var rooms = list.data;
			console.log("list.rooms->",JSON.stringify(list.data));
			if ( rooms && rooms.length > 0 ) {
...
var buildContactDiv = function(contactlistDivId, roster) {
	...
	for (i = 0; i < roster.length; i++) {
		...
		console.log("jid->",JSON.stringify(jid));
...
//webim构造联系人json数据
var buildContactJson = function(iroster) {
	console.log("iroster->",JSON.stringify(iroster));
	var item = [];
	
	for (i = 0; i < iroster.length; i++) {
		if (!(iroster[i].subscription == 'both' || iroster[i].subscription == 'from')) {
			continue;
		}
		var id = iroster[i].jid;
		console.log("id->",JSON.stringify(id));
		var name =iroster[i].name;
		console.log("name->",JSON.stringify(name));
		var face = "http://tp1.sinaimg.cn/1571889140/180/40030060651/"+i;
...
//webim构造群组列表json数据
var buildListRoomJson = function(irooms) {
	console.log("irooms->",JSON.stringify(irooms));
	var rItem = [];
	
	for (i = 0; i < irooms.length; i++) {
		var rName = irooms[i].name;
		var rId = irooms[i].roomId || irooms[i].id;
		var rFace = "images/"+i+1+".png";
		var room = new Object();
		room.id = rId;
		room.name = rName;
		room.face = rFace;
...
var handleTextMessage = function(message) {
	console.log("handleTextMessage-->message->",JSON.stringify(message));
...
var handleEmotion = function(message) {
	console.log("handleEmotion-->message->",JSON.stringify(message));
...
var handlePictureMessage = function(message) {
	console.log("handlePictureMessage-->message->",JSON.stringify(message));
...
var appendMsg = function(who, contact, message) {
	console.log("appendMsg-->who->",JSON.stringify(who));
	console.log("appendMsg-->contact->",JSON.stringify(contact));
	console.log("appendMsg-->message->",JSON.stringify(message));
	...
	console.log("lineDiv-->1111->",lineDiv);
	if (curChatUserId == null) {
	...
	if (curChatUserId && curChatUserId.indexOf(contact) < 0) {
		...
		console.log("contactLi-->1111->",contactLi);
	}
	var msgContentDiv = getContactChatDiv(contactDivId);
	console.log("msgContentDiv-->1111->",msgContentDiv);
	if (curUserId == who) {
	...
	console.log("msgContentDiv-->2222->",msgContentDiv);
	if(type == 'audio' && msg.audioShim) {
		setTimeout(function(){
	...
	console.log("msgContentDiv-->333->",msgContentDiv);
	console.log("lineDiv-->222->",lineDiv);
	return lineDiv;
};
...
//webim根据roomId构造room成员列表json
//根据roomId查询room成员列表
var queryOccupantsJson = function queryOccupantsJson(roomId) {

...
if ( contentType.indexOf("multipart/form-data") >= 0 )
{
	...
	while( fileItems.hasNext() )
	{
		...
		if ( fieldName.equals("__initParams") ){
			...
			String virtualPath = "uploadFiles/avatar/user_avatar" + avatarNumber + "_" + fileName + ".jpg";
			...
			if( isSourcePic ){
				...
				result.sourceUrl = virtualPath = String.format("uploadFiles/avatar/user_source_%s.%s", fileName, sourceExtendName);
			}else{
			//头像图片（默认的 file 域的名称：__avatar1,2,3...，可在插件配置参数中自定义，参数名：avatar_field_names）。
				result.avatarUrls.add(virtualPath);
				avatarNumber++;
			}
			String folderPath = application.getRealPath("/") + "uploadFiles/avatar/";
			File file =new File(folderPath);    
			//如果文件夹不存在则创建    
			if  (!file .exists()  && !file .isDirectory()){       
			    System.out.println(folderPath+"目录不存在");  
			    file.mkdirs();    
			}
			String filePath = application.getRealPath("/") + virtualPath;
			inputStream = new BufferedInputStream(fileItem.openStream());

...
public class TwoDimensionCode {
     ...
     * @param imgType 图片类型
     */
    public static void encoderQRCode(String content, String imgPath, String imgType, String size) {
    ...
    private static BufferedImage qRCodeCommon(String content, String imgType, int size) {
        ...
        try {
            ...
            bufImg = new BufferedImage(imgSize * 10, imgSize * 10,
                    BufferedImage.TYPE_INT_RGB);
            Graphics2D gs = bufImg.createGraphics();
            // 设置背景颜色
            gs.setBackground(Color.WHITE);
            gs.clearRect(0, 0, imgSize * 10, imgSize * 10);
            ...
            if (contentBytes.length > 0 && contentBytes.length < 800) {
                ...
                for (int i = 0; i < codeOut.length; i++) {
                    for (int j = 0; j < codeOut.length; j++) {
                        if (codeOut[j][i]) {
                            gs.fillRect(j * 3 * 10 + pixoff, i * 3 * 10 + pixoff, 3 * 10, 3 * 10);
                        }
                    }
                }
            } else {

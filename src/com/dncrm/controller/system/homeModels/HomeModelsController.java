package com.dncrm.controller.system.homeModels;
...
import com.dncrm.service.system.homeModels.HomeModelsService;
...
@Controller
@RequestMapping(value = "/homeModels")
public class HomeModelsController extends BaseController{

	@Resource(name = "homeModelsService")
	private HomeModelsService homeModelsService;
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value = "/homeModelsList")
	public ModelAndView modelsList(Page page) throws Exception{
		ModelAndView mv = new ModelAndView();
		PageData pd = this.getPageData();
		pd.put("elevator_id", "2");
		page.setPd(pd);
		List<PageData> modelsList = homeModelsService.listPageHomeModels(page);
		mv.addObject("models_name", pd.get("models_name"));
		mv.addObject("modelsList", modelsList);
		mv.addObject("page", page);
		mv.setViewName("system/homeModels/homeModels");
		return mv;
	}
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value = "/toHomeModelsAdd")
	public ModelAndView toHomeModelsAdd() throws Exception{
		ModelAndView mv = new ModelAndView();
		...
		mv.addObject("msg", "homeModelsAdd");
		mv.setViewName("system/homeModels/homeModels_edit");
		return mv;
	}
	 ...
	 * @throws Exception 
	 */
	@RequestMapping(value = "/toHomeModelsEdit")
	public ModelAndView toHomeModelsEdit() throws Exception{
		ModelAndView mv = new ModelAndView();
		PageData pd = this.getPageData();
		...
		try {
			pd = homeModelsService.findModelsById(pd);
	 ...
	 * @return
	 */
	@RequestMapping(value = "/homeModelsAdd")
	public ModelAndView homeModelsAdd(){
		ModelAndView mv = new ModelAndView();
		PageData pd = this.getPageData();
		
		try {
			homeModelsService.modelsAdd(pd);
	...
	public ModelAndView modelsEdit(){
		...
		PageData pd = this.getPageData();
		try {
			homeModelsService.modelsUpdate(pd);
	...
	public void modelsDelete(String models_id,PrintWriter out){
		...
		PageData pd = this.getPageData();
		try {
			homeModelsService.modelsDelete(pd);
			out.write("success");
			out.flush();
			out.close();
		} catch (Exception e) {
		...
		try {
			if(DATA_IDS !=null && !"".equals(DATA_IDS)){
				String[] ArrayDATA_IDS = DATA_IDS.split(",");
					...
					homeModelsService.modelsDeleteAll(ArrayDATA_IDS);
					pd.put("msg", "ok");
					pdList.add(pd);
					map.put("list", pdList);
...
//			countPrice = Arith.mul(countPrice, elevatorNum);
	...
	public Object findStoreyBySpeedId() throws Exception{
		...
		if(models_id!=null && !"".equals(models_id) && (flag.equals("1")|| flag.equals(""))){
			List<PageData> modelsList = homeModelsService.findModelsByTypeList(pd);
	...
	//计算电梯标准价格
	@RequestMapping(value = "countHomeStandardPrice")
	@ResponseBody
	public Object countHomeStandardPrice() throws Exception{
		PageData pd = this.getPageData();
		JSONObject result = new JSONObject();
		pd = homeElevatorStandardService.countHomeStandardPrice(pd);
		if(pd!=null){
			if(pd.get("home_standard_price")!=null){
				result.put("success", true);
				result.put("elevator_standard_price", pd.get("home_standard_price"));
			}else{
